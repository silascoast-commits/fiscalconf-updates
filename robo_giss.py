...
# ===========================================================================
    ...
    def _clicar_encerrar(self, page, modulo, tipo):
        ...
        # Pré-verificação: detecta se o elemento existe no frame ANTES de
        # entrar em expect_page, evitando espera desnecessária de 15s.
        script_check = r"""(termos) => {
            function norm(s) {
                return (s||'').trim().toLowerCase()
                    .normalize('NFD').replace(/[̀-ͯ]/g,'')
                    .replace(/\s+/g,' ');
            }
            var sels = 'a,button,input[type="button"],input[type="submit"],' +
                       'input[type="image"],td[onclick],span[onclick],div[onclick]';
            for (var el of document.querySelectorAll(sels)) {
                ...
                if (txt.length > 0 && txt.length < 300 &&
                        termos.some(function(t){ return txt.indexOf(t) >= 0; }))
                    return true;
            }
            return false;
        }"""

        # Script JS: preenche mes/ano E clica o elemento no mesmo tick.
        # Busca em a, button, input E td/span/div com onclick — não só <a>.
        script_click = r"""([mes, ano, termos]) => {
            ...
            // Encontra o elemento pelo texto (a, button, input, td onclick…)
            var sels = 'a,button,input[type="button"],input[type="submit"],' +
                       'input[type="image"],td[onclick],span[onclick],div[onclick]';
            for (var el of document.querySelectorAll(sels)) {
                var txt = norm(el.innerText || el.textContent || el.value || '');
                ...
                if (termos.some(function(t){ return txt.indexOf(t) >= 0; })) {
                    ...
                    el.dispatchEvent(new MouseEvent('mouseup',    {bubbles:true, button:0}));
                    el.click();
                    return {ok: true, tag: el.tagName, txt: txt, href: href, oc: oc,
        ...
        for f in self._todos_frames(page):
            ...
            res = None
            try:
                # Pré-verifica presença do elemento — evita timeout de 15s à toa
                existe = f.evaluate(script_check, termos_busca)
                if not existe:
                    self._log("[{}] '{}' ausente no frame {} — pulando.".format(
                ...
                try:
                    ...
                    if not res or not res.get("ok"):
                        ...
                        continue
                    self._log("[{}] Link clicado — tag={} mes={} ano={} oc={}".format(
                        modulo, res.get("tag"), res.get("mes"), res.get("ano"), res.get("oc","")[:80]))
                    ...
                    if res and res.get("ok"):
                        self._log("[{}] Link clicado sem popup — tag={} mes={} ano={} oc={}".format(
                            modulo, res.get("tag"), res.get("mes"), res.get("ano"), res.get("oc","")[:80]))
                        return True
                    self._log("[{}] Link não encontrado no frame {}.".format(modulo, f.url[:50]))
        ...
        # Fallback final: _clicar_link genérico em todos os frames
        self._log("[{}] JS click falhou — tentando _clicar_link genérico para '{}'.".format(modulo, tipo))
        termos_fallback = termos_busca[0] if termos_busca else tipo
        if self._clicar_link(page, termos_fallback):
            self._log("[{}] Clicado via _clicar_link fallback: '{}'.".format(modulo, termos_fallback))
            return True
